      ...
      describe("when action is a Cheerio task correct actions are dispatched", () => {
        ...
        beforeEach(() => {
          ...
          const defaultResponse =
            "<!doctype html><html><body><div>text</div></body></html>";
        ...
        it("should dispatch ACTION_PENDING once", () => {
          ...
          return store.dispatch(defaultCheerioAction).then(res => {
            expect(
              store.getActions().filter(e => e.type === "CHEERIO_TASK_PENDING")
                .length
            ).to.equal(1);
          });
        });
        ...
        it("should dispatch ACTION_FULFILLED once", () => {
          ...
          return store.dispatch(defaultCheerioAction).then(res => {
            expect(
              store
                .getActions()
                .filter(e => e.type === "CHEERIO_TASK_FULFILLED").length
            ).to.equal(1);
          });
        });
        ...
        it("ACTION_FULFILLED should contain parsed data", () => {
          ...
          return store.dispatch(defaultCheerioAction).then(res => {
            expect(
              store
                .getActions()
                .filter(e => e.type === "CHEERIO_TASK_FULFILLED")[0].payload
                .parsedData
            ).to.equal("text");
          });
        });
        ...
        it("should dispatch ACTION_REJECTED once", () => {
          ...
          return store.dispatch(defaultCheerioAction).then(res => {
            expect(
              store.getActions().filter(e => e.type === "CHEERIO_TASK_REJECTED")
                .length
            ).to.equal(1);
          });
        });
        ...
        it("ACTION_REJECTED should contain error object", () => {
          ...
          return store.dispatch(defaultCheerioAction).then(res => {
            expect(
              store
                .getActions()
                .filter(e => e.type === "CHEERIO_TASK_REJECTED")[0].payload.err
            ).to.be.defined;
          });
        });
